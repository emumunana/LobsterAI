...
**Given** 主会话中的某个 assistant turn 关联了 subagent run  
**When** 用户点击该 turn 下方的 subagent chip  
...
- 主会话 assistant turn 根据 tool call id 关联 subagent run。
- turn 下方展示 subagent chip。
- chip 左右边界和 assistant 正文内容区域对齐。
...
- 点击主会话中的 subagent chip，不离开主会话页面，右侧 panel 展示该 subagent 详情。
